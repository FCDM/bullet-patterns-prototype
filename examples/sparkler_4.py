...
class Comet:
	...
	def spawn(self, time):
		if time % Comet.SPAWN_INTERVAL != 0:
			return
		angle = self.local_time/self.ROTATION_SPEED*2*math.pi
		for i in range(Comet.SPAWNERS):
			self.sparkles.append(
				Bullet(
					self.x, 
					self.y, 
					Bullet.BULLET_SPEED*math.cos(angle + 2*math.pi/Comet.SPAWNERS*i) + self.vx, 
					Bullet.BULLET_SPEED*math.sin(angle + 2*math.pi/Comet.SPAWNERS*i) + self.vy
					)
				)
